...
public class WaveManager : MonoBehaviour {
	...
	void Update() {
		...
		if (spawnedThisWave == totalToSpawnForWave && enemiesAlive == 0) {
			...
			if(waveNumber==10 ||waveNumber==20||waveNumber==30 || waveNumber==40 || waveNumber==50 || waveNumber==60 || waveNumber==70)
			{
				waveNumber=1;
			}
			Time.timeScale=0;
	...
	public void Pause()
	{
		...
		Debug.Log ("Pause enter" + EventSystem.current.currentSelectedGameObject.name);
		Time.timeScale=0;
		//backgroundAudio = GameObject.Find("BackgroundMusic").GetComponent<AudioSource>();
		backgroundAudio.Pause();
	}
	...
	void CreateWaves() {
		Wave wave1 = new Wave();
		wave1.entries.Add(new Wave.Entry(enemies[0], 20 * difficulty));
		waves.Add(wave1);
		
		Wave wave2 = new Wave();
		wave2.entries.Add(new Wave.Entry(enemies[0], 30 * difficulty));
		waves.Add(wave2);
		
		Wave wave3 = new Wave();
		wave3.entries.Add(new Wave.Entry(enemies[0], 25 * difficulty));
		wave3.entries.Add(new Wave.Entry(enemies[1], 25 * difficulty));
        waves.Add(wave3);

		Wave wave4 = new Wave();
		wave4.entries.Add(new Wave.Entry(enemies[0], 30 * difficulty));
		wave4.entries.Add(new Wave.Entry(enemies[1], 30 * difficulty));
		waves.Add(wave4);
