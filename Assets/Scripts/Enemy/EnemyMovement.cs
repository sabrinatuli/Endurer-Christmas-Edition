...
public class EnemyMovement : MonoBehaviour {
			...
			if (playerHealth.currentHealth > 0) {
				...
				if (Physics.Raycast (shootRay, out shootHit, 25, shootableMask)) {
					if (shootHit.transform.tag == "Player") {
						...
						//myRenderer.materials[0].SetColor("_RimColor", Color.Lerp(myRenderer.materials[0].GetColor("_RimColor"), new Color(1, 0, 0, 1), 2 * Time.deltaTime));
						...
						//myRenderer.materials[0].SetColor("_RimColor", Color.Lerp(myRenderer.materials[0].GetColor("_RimColor"), new Color(0, 0, 0, 1), 2 * Time.deltaTime));
					}
				}
			}
	...
	void SetRandomNavTarget() {
		Vector3 randomPosition = Random.insideUnitSphere * 90;
		randomPosition.y = 0;
		randomPosition += transform.position;
		NavMeshHit hit;
		hasValidTarget = NavMesh.SamplePosition(randomPosition, out hit, 400, 25);
