...
public class KrampusAttack : MonoBehaviour {
	...
	void Attack() {
		...
		if (attackCount < attacksPerSpecialAttack) {
			if (bulletTimer > timeBetweenBullets && bulletCount < bulletsPerVolley) {
				...
				//Instantiate(bullet, transform.position + new Vector3(0, 0.5f, 0), rot); tuli
				Instantiate(bullet, transform.position + new Vector3(0, 3f, 0), rot);
